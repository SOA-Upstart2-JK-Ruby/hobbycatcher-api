...
namespace :worker do
  namespace :run do
    ...
    task :dev => :config do
      sh 'RACK_ENV=development bundle exec shoryuken -r ./workers/courses_get_worker.rb -C ./workers/shoryuken_dev.yml'
    end

    desc 'Run the background cloning worker in testing mode'
    task :test => :config do
      sh 'RACK_ENV=test bundle exec shoryuken -r ./workers/courses_get_worker.rb -C ./workers/shoryuken_test.yml'
    end

    desc 'Run the background cloning worker in production mode'
    task :production => :config do
      sh 'RACK_ENV=production bundle exec shoryuken -r ./workers/courses_get_worker.rb -C ./workers/shoryuken.yml'
    end
  end
end
